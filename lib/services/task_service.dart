...
class TaskService {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // Print fetched tasks for debugging
      for (var item in items) {
        print('Fetched Todo: ${item['title']}, '
            'Description: ${item['description']}, '
            'Completed: ${item['is_completed']}, '
      ...
      Map<String, Task> existingTaskMap = {
        for (var task in existingTasks)
          if (task.apiId != null) task.apiId!: task // Use non-null API IDs only
      };
      ...
      for (var item in items) {
        ...
        if (apiId == null) {
          ...
          continue; // Skip tasks with null API IDs
        }
  ...
  Future<void> deleteTaskFromAPI(String apiId) async {
    final url = '$apiUrl/$apiId'; // Construct API URL with task ID
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Delete task from the local database using the API ID
   ...
   // Method to update task in API and then in local database
  ...
  // Method to delete all tasks from the API
      ...
      if (response.statusCode == 200) {
        ...
        for (var item in items) {
          ...
          if (deleteResponse.statusCode == 200) {
            ...
            print('Failed to delete task with ID $apiId: ${deleteResponse.body}');
          }
        }
      } else {
